...
?>                  
        // Organigrama del Cuerpo Policial
        // json must be in 'oc_data' global variable
        ...
        var oc_style = {
            ...
            inner_padding      : 5,                     // space from text to box border
            ...
            title_font_size    : 10,                     // size of font used for displaying titles inside boxes
            subtitle_font_size : 10,                     // size of font used for displaying subtitles inside boxes
            title_char_size    : [ 10, 15 ],              // size (x, y) of a char of the font used for displaying titles
            subtitle_char_size : [ 5, 15 ],              // size (x, y) of a char of the font used for displaying subtitles
            max_text_width     : 25,                     // max width (in chars) of each line of text ('0' for no limit) 
        ...
        function oc_box_click_handler (event, box) {
            box.unclick();
            Ext.Ajax.request({
                url: BASE_URL+'cuerpo_policial/departamentos/detail?id='+box.id,
                method: 'GET',
                success: function(response){
                    eval(response.responseText); 
                        box.click(function(event){
                            oc_style.box_click_handler(event,box);
                        });                        
                },
                failure: function(){
                    Ext.Msg.alert('Falla','fallo la respuesta AJAX');
                    box.click(function(event){
                            oc_style.box_click_handler(event,box);
                    }); 
                 }
    ...
    if (node.children !== undefined) {
        for (var i = 0; i < node.children.length; i++) {
            ...
            if (child.position == 'left') {
                ...
                if (collateral_children == 1)   // patch for case of only one collateral children drawn at left of the node
                    node.fullbbox[0] += 1 * node.boundbox[0] ;
            }
            else {   // right
                child.deltacorner = [
...
function oc_draw_obj (node, parent, xoffset, yoffset) {
    ...
    box.hover(
        function (event) {
            this.attr({
                fill: oc_style.box_color_hover
            });
        },
        function (event) {
            this.attr({
                fill: oc_style.box_color
            });
        }
        //, overScope, outScope
        );
...
var panel_organigrama= new Ext.Panel({
        layout: 'fit',
        autoHeight: true,
        autoWidth: true,
        //autoScroll: true,
	...
	items:[{html:'<div id="oc_container"></div> '}]
    });

<?php
    if($replace == 'window'){
        $wdata['w_item'] = 'panel_organigrama';
