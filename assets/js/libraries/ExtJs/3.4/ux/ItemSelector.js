...
Ext.ux.form.ItemSelector = Ext.extend(Ext.form.Field,  {
    ...
    onRender: function(ct, position){
        ...
        this.iconClear = this.imagePath + (this.iconClear || 'clear_list.gif');
        var el=icons.getEl();
      
		
	  
        if(!this.hideNavIcons){
    ...
    fromTo : function() {
        ...
        if (selectionsArray.length > 0) {
            ...
            for (var i=0; i<records.length; i++) {
				...
				cont=0;
				if(this.dynaViews){
						for(var k=0; k< this.toMultiselect.view.store.data.items.length; k++){
							cont+=(this.toMultiselect.view.store.data.items[k].data.value==record.data.value)?1:0;
						}
						if(cont==0){
    ...
    toFrom : function() {
        ...
        if (selectionsArray.length > 0) {
            ...
            for (var i=0; i<records.length; i++) {
                ...
                this.toMultiselect.view.store.remove(record);
                if(this.dynaViews){
					for(var k=0; k< this.fromMultiselect.view.store.data.items.length; k++){
							cont+=(this.fromMultiselect.view.store.data.items[k].data.value==record.data.value)?1:0;
					}
					if(cont==0){
    ...
    reset: function(){
		...
		if(this.dynaViews){
			for(var i=0; i<range.length; i++){
				...
				cont=0;
				for(var k=0; k< this.fromMultiselect.view.store.data.items.length; k++){
					cont+=(this.fromMultiselect.view.store.data.items[k].data.value==rec.data.value)?1:0;
				}
				if(cont==0) this.fromMultiselect.view.store.add(rec);

			}
		}else this.fromMultiselect.store.add(range);
        var si = this.fromMultiselect.store.sortInfo;
        if (si){
            this.fromMultiselect.store.sort(si.field, si.direction);
        }
